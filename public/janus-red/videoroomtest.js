...
$(document).ready(function() {
	...
	Janus.init({debug: "all", callback: function() {
		...
		$('#start').one('click', function() {
			...
			// Make sure the browser supports WebRTCi
...
function registerUsername() {
	if($('#username').length === 0) {
		...
		var register = { "request": "join", "room": myroom, "ptype": "publisher", "display": username, "pin": mypin };
		myusername = username;
	...
	janus.attach(
		{
			...
			success: function(pluginHandle) {
				...
				var listen = { "request": "join", "room": myroom, "ptype": "subscriber", "feed": id, "private_id": mypvtid, "pin": mypin };
					...
					remoteFeed.createAnswer(
						{
							...
							success: function(jsep) {
								Janus.debug("Got SDP!");
								Janus.debug(jsep);
								var body = { "request": "start", "room": myroom, "pin": mypin };
